...
install.packages("ggplot2")   # install packages (only need to run this the first time you use this code)
...
swp_plot <- ggplot(swp, aes(x = block, y = reading_bar, color = block)) +
              ...
              geom_segment(aes(x = 0.6, xend = 1.4, y = -6.3, yend = -6.3), 
                           linetype = "dashed", linewidth = 1.25, color = "black") + # DRI baseline SWP horizontal bar
              geom_segment(aes(x = 1.6, xend = 2.4, y = -6.1, yend = -6.1), 
